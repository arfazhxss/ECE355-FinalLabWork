...
//
static unsigned char Characters[][8] = {
...
void myGPIOB_Init()
{
    ...
    GPIOB->MODER &= ~(GPIO_MODER_MODER4 | GPIO_MODER_MODER6 | GPIO_MODER_MODER7);      // Clear the bits
    GPIOB->MODER |= (GPIO_MODER_MODER4_0 | GPIO_MODER_MODER6_0 | GPIO_MODER_MODER7_0); // Set bits to output (01 = output)
    GPIOB->OTYPER &= ~(GPIO_OTYPER_OT_4 | GPIO_OTYPER_OT_6 | GPIO_OTYPER_OT_7);        // Set the output type as push-pull
    // Configure high-speed mode (11 - high-speed)
    GPIOB->OSPEEDR |= (GPIO_OSPEEDER_OSPEEDR4 | GPIO_OSPEEDER_OSPEEDR6 | GPIO_OSPEEDER_OSPEEDR7);
    GPIOB->PUPDR &= ~(GPIO_PUPDR_PUPDR4 | GPIO_PUPDR_PUPDR6 | GPIO_PUPDR_PUPDR7); // Ensure no pull-up/pull-down

    // Configure AF0 for PB3
    GPIOB->MODER &= ~(GPIO_MODER_MODER3);     // Clear PB3 bits
    GPIOB->MODER |= GPIO_MODER_MODER3_1;      // Set PB5 bits as alternate function (10 = AF0)
    GPIOB->OSPEEDR |= GPIO_OSPEEDER_OSPEEDR3; // Set at high-speed
    GPIOB->PUPDR &= ~(GPIO_PUPDR_PUPDR3);     // Ensure no pull-up/pull-down

    // Configure AF0 for PB5
    GPIOB->MODER &= ~(GPIO_MODER_MODER5);     // Clear PB5 bits
    GPIOB->MODER |= GPIO_MODER_MODER5_1;      // Set PB5 bits as alternate function (10 = AF0)
    GPIOB->OSPEEDR |= GPIO_OSPEEDER_OSPEEDR5; // Set at high-speed
...
void TIM3_delay(uint8_t milliseconds)
{
    ...
    // Wait for the timer to reach the given amount of time
...
// The frequency number is to reference a device which generates square waves
// In this project, 1 - NE555 timer, 2 - Function generator
...
void refresh_OLED_test(void)
{
    ...
    while (count <= 200)
    {
        ...
        {
            // Update the buffer and then print it out
            snprintf((char *)Buffer, sizeof(Buffer), "PEEKABOO!");
            oled_print(Buffer, 1, STARTING_COL);

            snprintf((char *)Buffer, sizeof(Buffer), "I see you!");
            oled_print(Buffer, 3, STARTING_COL);

            snprintf((char *)Buffer, sizeof(Buffer), "Count: %d", count);
            oled_print(Buffer, 5, STARTING_COL);
